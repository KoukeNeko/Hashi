...
export const StatCard: React.FC<StatCardProps> = ({ title, value, subValue, icon: Icon, colorClass = "text-emerald-500", trend }) => {
  return (
    <div className="bg-surface border border-border p-6 rounded-lg shadow-lg hover:border-emerald-500/30 transition-colors">
      ...
      {trend && (
        <div className={`mt-2 text-xs font-mono flex items-center ${trend === 'up' ? 'text-emerald-500' : trend === 'down' ? 'text-rose-500' : 'text-zinc-500'}`}>
