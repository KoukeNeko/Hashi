            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {MOCK_VMS.map((vm) => (
                    <div key={vm.id} className="bg-surface border border-border rounded-lg p-5 hover:border-zinc-600 transition-colors relative overflow-hidden group shadow-lg">
                        {/* Status Strip */}
                        <div className={`absolute top-0 left-0 w-1 h-full ${vm.status === VmStatus.RUNNING ? 'bg-emerald-500' :
                        ...
                        <div className="flex justify-between items-start mb-4 pl-3">
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    <span className={`w-2 h-2 rounded-full ${vm.status === VmStatus.RUNNING ? 'bg-emerald-500' :
                                            vm.status === VmStatus.SHUTOFF ? 'bg-rose-500' :
                                                'bg-amber-500'
                                        }`}></span>
