...
const NginxManager: React.FC = () => {
    ...
    const [selectedHostForConfig, setSelectedHostForConfig] = useState<string | null>(null);
    const [isSslDialogOpen, setIsSslDialogOpen] = useState(false);
    ...
    const isNginxInstalled = status && status.version && status.version !== 'unknown';
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {!isNginxInstalled ? (
                ...
                <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {hosts.length === 0 ? (
                                ...
                                hosts.map((host) => (
                                    <div
                                        key={host.name}
                                        className="bg-surface border border-border rounded-lg p-5 shadow-lg group hover:border-emerald-500/30 transition-colors"
                                    >
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="flex items-center gap-3">
                                                <div
                                                    className={`p - 2 rounded - lg ${host.enabled ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-800 text-zinc-500'} `}
                                                >
                                                    {host.type === 'proxy' ? (
                                                        <ArrowUpDown size={20} />
                                                    ) : (
                                            ...
                                            <div className="flex flex-col items-end gap-1">
                                                ...
                                                <span
                                                    className={`px - 2 py - 0.5 rounded text - [10px] uppercase font - bold tracking - wide border ${getTypeColor(host.type)} `}
                                                >
                                        ...
                                        <div className="space-y-2 mb-6">
                                            ...
                                            <div className="flex justify-between text-sm">
                                                ...
                                                <span
                                                    className={`font - mono text - xs ${host.sslEnabled ? 'text-emerald-400' : 'text-amber-500'} `}
                                                >
                                        ...
                                        <div className="flex justify-between items-center pt-4 border-t border-border">
                                            <button
                                                onClick={() => setSelectedHostForConfig(host.name)}
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-zinc-200 break-all">
                                                        {cert.domain}
                                                    </h3>
                                        ...
                                        <div className="flex justify-between items-center pt-4 border-t border-border">
                                            ...
                                            <div className="flex gap-2">
                                                <button
                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-emerald-400 rounded text-xs font-medium transition-colors"
                                                    title="Renew"
                                                >
                                                    <RefreshCw size={14} /> Renew
                                                </button>
                                                <button
                                                    className="p-1.5 hover:bg-rose-500/10 text-zinc-400 hover:text-rose-400 rounded transition-colors"
                                                    title="Revoke"
                                                >
                                                    <Trash2 size={16} />
                                                </button>
                    ...
                    {/* Edit Config Dialog */}
                    <EditConfigDialog
                        isOpen={!!selectedHostForConfig}
                        hostName={selectedHostForConfig}
                        onClose={() => setSelectedHostForConfig(null)}
                        onSuccess={loadData}
                    />

                    {/* SSL Request Dialog */}
                    <SslRequestDialog
                        isOpen={isSslDialogOpen}
                        onClose={() => setIsSslDialogOpen(false)}
                        onSuccess={loadData}
                        hosts={hosts}
                    />
                </>
